...
export async function registerOperator(formData: FormData) {
  ...
  if (!city?.countryId) {
    throw new Error(`City ${cityId} has no country assigned`);
  }

  // Verificar email único antes de crear nada
  const existing = await prisma.user.findUnique({ where: { email } });
  if (existing) {
    // Email ya registrado — en producción sería un error visible al usuario
    return;
  }
  ...
  if (file && file instanceof File && file.size > 0) {
    ...
    if (res.ok) {
      ...
      if (json.url) licenseDocumentUrl = json.url;
    }
  }

  const operatorType = operatorTypeFromPrestadorTipo(prestadorTipo);
  const role = operatorType === 'AGENCY' ? 'OPERATOR_AGENCY' : 'OPERATOR_FREELANCE';

  const hashedPassword = await bcrypt.hash(password, 12);

  // Crear User y Operator en una transacción para consistencia
  ...
  // Crear cookie de sesión — el operador entra directo al dashboard
  const token = signAuthToken({
    userId:     user.id,
    email:      user.email,
    role:       role as import('@/lib/auth').AuthRole,
